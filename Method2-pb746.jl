...
#Part 2: ODE run 
#2.1 Deterministic Run and visualisation
u0 = [:I => 100, :R => 300, :L => 100] #initial conditions of proteins
...
#2.2 Stochastic Run and visualisation
nsim = 100
u0 = [:I => 100, :R => 300, :L => 100]
tspan = (0.0,10.0)
p = Dict(:α_0=> 85,:β_0=>100, :γ=>125, :δ_I=>2,:δ_L=>10)
...
histogram(dflastpoint[:,2], title = "% simulations vs Rss range |\n Initial conc: $(u0) |\n % High Rss (Rss > $(co)) = $(count_highR) ", xlabel = "Rss", ylabel = "% simulations", label = false)
...
if (length(simHighR)>2) & (length(simLowR)>2)
    ...
    plot!(plot_title = "Initial conc: $(u0), threR = $(co)", plot_titlefontsize = 12, size = (600,600))
    plot!(ylim = (0,maxSS))
else
    p6_1 = plot(palette =:Spectral_4, arrayrun[1:4,1],layout= (3,1), title = ["SinI, $(u0[1]), ss = $(mean(dflastpoint[:,1]))+/- $(ssIsd)" "SinR, $(u0[2]), ss = $(mean(dflastpoint[:,2]))+/- $(ssRsd)" "SlrR, $(u0[3]), ss = $(mean(dflastpoint[:,3]))+/- $(ssLsd)" ], legend = false)
end
...
for i in 1:length(di_sp)
    for j in 1:length(dl_sp)
        ...
        dl = pars[:δ_L] 
        c0 = b
        c1 = di*b + dl*b -1 -di*a - dl*g
        c2 = di*dl*b + b - di - dl - di*dl*a - di*dl*g
...
#3.4 visualise
#3.4.1 plot umber of real positive Rss
mycmap = [:black, :grey]
...
HeatMapmonoRsszoom = heatmap(dl_sp,di_sp,didl_monoRss, color = mycmap2,size = (600,600),xlabel = "δL", ylabel = "δI", title = "The value of mono Rss \n α0, β0, γ = $(pars[:α_0]), $(pars[:β_0]), $(pars[:γ])\n scanning δI over 0.0 - $(di_sp[end]) \nand δL over 0.0 - $(dl_sp[end])",guidefontsize = 15,xtickfontsize=10, ytickfontsize = 10, yticks = di_sp[1:100:end], xticks = dl_sp[1:100:end], tick_direction = :out, ylims = (0,2))
contour!(dl_sp,di_sp,didl_monoRss,color = :white, lw = 0.5)
...
#I actually got a problem as finding eigenvalues of lowRss gave one positive eigenvalue, whihc contradict to the stable nature of lowRss fixed point
...
#Summary: at the end the we found that using δI = 2 and δL = 10 is the sensible choice as it produces saddle-node bifurcation points, also referred to as limit points (LP). The high Rss is 
#and we have the evidence that δL may be around 5.6 times higher than δI based on the binding constant
