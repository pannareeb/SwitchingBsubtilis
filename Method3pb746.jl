...
plot(discrete_t,sumEachTime)
sumEachTime_n = sum(soln,dims=2)
compare_b_wholesig = plot(TimeInterval,[spreadYFPnorm_sum',spreadRFPnorm_sum'],lw = 2, color = [:goldenrod :red4], label = ["YFP-Motile" "RFP-Matrix"], legendposition = :topleft,title = "Whole-biofilm signals vs Model prediction of bacteria density (b(t))", xlab = "Time (hrs)", ylab = "Intensity", xticks = TimeInterval[1:20:end], titlefontsize = 10)
plot!(twinx(),discrete_t,sumEachTime, label = "b(t)", legendposition = :bottomright,lw = 2, ylab = "b(t)")
compare_b_sumsig = plot(TimeInterval,[0.12.*spreadYFPnorm_sum' .+ 0.08.*spreadRFPnorm_sum'],lw = 2, color = :green, label = "Scaled sum", legendposition = :topleft,title = "Whole-biofilm signals vs Model prediction of bacteria density (b(t))", xlab = "Time (hrs)", ylab = "Intensity", xticks = TimeInterval[1:20:end], titlefontsize = 10)
plot!(twinx(),discrete_t,sumEachTime, label = "b(t)", legendposition = :bottomright,lw = 2, ylab = "b(t)")
#2.2 make a gif of model predictions
...
anim_bn = @animate for i in 1:4:length(discrete_t)
    p1 = plot(discrete_x,solb[i, 1:end], ylim = (minimum(solb),maximum(solb)),title="Bacteria at Time = $(round(discrete_t[i], digits = 1)) hr", ylab = "Bacteria density (b)",xlab = "Distance from one edge (x)",markershape = :circle,lw=2, legend = false, color = mycolt[i], markersize = 1, markerstrokewidth = 0)
    annotate!([(19,yannob, ("b0 = $(bstart)\nx0 = $(x0)\nDb = $(round(D_b,digits = 3))\nK = $(K)\nkd = $(kd)\nkg = $(kg)", 8, :blue))])
    p2 = plot(discrete_x,soln[i, 1:end], ylim = (minimum(soln),maximum(soln)),title= "Nutrient at Time = $(round(discrete_t[i], digits = 1)) hr", ylab = "Nutrient level (n)", xlab = "Distance from one edge (x)",markershape = :diamond,lw=2, legend = false, color = mycolt[i], markersize = 1, markerstrokewidth = 0)
    annotate!([(19,yannon, ("n0 = $(nstart)\nDn = $(D_n)\nθ = $(θ)\nω = $(ω)", 8, :blue))])
    plot(p1,p2, layout = (2,1), size = (500,500) )
end
gif(anim_bn,fps = 16, "NuBacSim_bn_out.gif")
